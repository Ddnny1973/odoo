...
Sub CallConsolaWithParameters()
    ...
    strPostData = "<?xml version=""1.0""?>" & _
              ...
              "<param><value><string>Api_2024</string></value></param>" & _
              "<param><value><string>mb_asesores.vencimientos</string></value></param>" & _
              "<param><value><string>lanzar_consola_job</string></value></param>" & _
              "<param>" & _
              "<value><struct>" & _
    ...
    ' Manejo especial para respuesta asíncrona tipo {"status": "running", ...}
    If InStr(strResponse, Chr(34) & "status" & Chr(34)) > 0 And InStr(strResponse, "running") > 0 Then
        MsgBox "El proceso de consola fue lanzado exitosamente en Odoo y se está ejecutando en segundo plano." & vbCrLf & _
               "Puede continuar trabajando en Excel. Recibirá notificación cuando finalice.", vbInformation, "Consola lanzada en background"
        GoTo CleanExit
    End If
