...
class Vencimientos(models.Model):
    ...
    def variables(self, month=None, year=None, hojas=None, control=None):
        ...
        return status, month, year, hojas, mensajes_whatsapp, control_mails, id_mb, google_drive_config, creds, gc, servicio_drive, lista_archivos
    ...
    def get_archivo(self, google_drive_config, servicio_drive, gc, df_sheets, sheet, poliza, mes, lista_archivos):
        ...
        if len(resultados) > 0:
            ...
            _logger.info(f"Antes de actualizr sheet ************")
            result = sheet.update_cell(268, 29, url) 
            _logger.info(f"get_archivo: result url: {result} ************")
            # result = google_drive_config.update_cell(self, sheet, row, column_nombrearchivo, nombrearchivo)

        return True

    @api.model
    def actualizar_modelo_vencimientos(self, google_drive_config, servicio_drive, gc, df_sheets, sheet, mes_FILTRO, ramo, lista_archivos):
        _logger.info(f"Dentro de Actualizando modelo vencimientos ************")
        ...
        for index, row in df_filtrado.iterrows():
            ...
            actualizar = self.get_archivo(google_drive_config, servicio_drive, gc, df_sheets, sheet, poliza, mes_FILTRO, lista_archivos)
        return df_filtrado

    @api.model
    def descarga_vencimientos(self, month=None, year=None, hojas=None, control=None):
        status, month, year, hojas, mensajes_whatsapp, control_mails, id_mb, google_drive_config, creds, gc, servicio_drive, lista_archivos = self.variables(month, year, hojas, control)
        duracion = 0
        ...
        if status == 'idle' or status == '' or status == None:
            ...
            try:
                ...
                nombre_hoja = 'PRUEBAS'
                df_sheets, sheet, sheet_df_dict = google_drive_config.cargar_hoja(nombre_hoja, gc)

                _logger.info(f"Antes de entrar a actualizar modelo ************")
                self.actualizar_modelo_vencimientos(google_drive_config, servicio_drive, gc, df_sheets, sheet, mes_FILTRO, nombre_hoja, lista_archivos)
