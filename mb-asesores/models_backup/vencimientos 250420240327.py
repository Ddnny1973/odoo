...
class Vencimientos(models.Model):
    ...
    # _inherit = 'mb_asesores.google.drive.config'
    ...
    def descarga_vencimientos(self, month=None, year=None, hojas=None, control=None):
        ...
        if control:
            ...
            mensajes_whatsapp = control[0]
            control_mails = control[1]  
        ...
        ¡¡Feliz Día!!
                ...
                for hoja in hojas:
                    for record in self.search([('ramo', '=', hoja)]):
                        ...
                        if id_archivo_adjunto and record.mail_status == 'pendiente' and record.ramo == hoja:
                            ...
                            # _logger.info(f"******destinatario: {destinatario} poliza {record.id_poliza}")
                            destinatario = record.email_to
    ...
    def consultar_vencimientos(self, mes):
        ...
        # vencimientos_ids = self.search([('mes', '=', mes)])
        # vencimientos_data = self.read(vencimientos_ids)
    ...
    #     _logger.info('Entrando en la función actualizar_sheet_vencimientos')
    #     id_mb = self.env['res.partner'].search([('name', '=', 'MB-Asesores')]).id
    #     _logger.info(f"ID del administrador: {id_mb}")
    ...
    #     # Recurriendo el df para actualizar
    #     hojas = hojas.split(",")
    #     hojas = [hoja.strip() for hoja in hojas]

    #     df = self.consultar_vencimientos(mes_FILTRO)
            
    #     for nombre_hoja in hojas:
    #         _logger.info(f"Ejecutando hoja: {nombre_hoja}")
    #         sheet = google_drive_config.cargar_hoja(nombre_hoja, gc)
            
    #         # Buecando columnas
    #         columna_estado_correo = google_drive_config.buscando_columna('ESTADO CORREO', sheet)
    #         time.sleep(1)
    #         columna_estado_whatsapp = google_drive_config.buscando_columna('ESTADO WHATSAPP', sheet)
    #         time.sleep(1)

    #         # Obtener todas las filas donde el mes es "Marzo"
    #         columna_mes = google_drive_config.buscando_columna('MES', sheet)
    #         time.sleep(1)
    #         columna_poliza = google_drive_config.buscando_columna('POLIZA', sheet)
    #         time.sleep(1)

    #         ## Filtrando df para mensajes de correo enviado
    #         df_filtered = df[(df['ramo'] == nombre_hoja)
    #                         # & (df['mail_status'] == 'enviado')
    #                         ]
    ...
    #         #     _logger.info(row['id_poliza'], row['ramo'], row['mail_status'])

    #             # Buscando la posición de las columnas de control
    #             poliza = row['id_poliza'].lstrip('0')

    #             result = google_drive_config.actualizar_registro(nombre_hoja,
    #                                         poliza, 
    #                                         row['mail_status'],
    #                                         row['whatsapp_status'],
    #                                         sheet,
    ...
    #                                         columna_poliza,
    #                                         mes_FILTRO)
    #             _logger.info(f"result {result} poliza {poliza} len(poliza) {len(poliza)}")
    #             time.sleep(15)
    ...
    def actualizar_sheet_vencimientos(self, month=None, year=None, hojas=None):
        ...
        for nombre_hoja in hojas:
            ...
            sheet_df, sheet = self.cargar_hoja(nombre_hoja, gc)
            ...
            for index, row in df_filtered.iterrows():
                # Mostrar el valor del campo 'id_poliza' para cada fila
                print(row['id_poliza'], row['ramo'], row['mail_status'])

                # Buscando la posición de las columnas de control
                poliza = row['id_poliza'].lstrip('0')
                status = row['mail_status']
                status_whatsapp = row['whatsapp_status']
                self.actualizar_registro(nombre_hoja, month, poliza, status, status_whatsapp, sheet_df, sheet)
                time.sleep(1)
            time.sleep(5)
