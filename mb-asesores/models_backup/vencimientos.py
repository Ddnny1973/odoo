...
class Vencimientos(models.Model):
    ...
    def get_archivo(self, google_drive_config, servicio_drive, gc, df_sheets, sheet, poliza, mes, lista_archivos):
        ...
        if len(resultados) > 0:
            ...
            try:
                result = sheet.update_cell(267, 28, 'url')
    ...
    @api.model
    def actualizar_modelo_vencimientos(self, df_sheets, year, mes_FILTRO, ramo):
        ...
        _logger.info(f"df_filtrado: {df_filtrado.count()}")
        for index, row in df_filtrado.iterrows():
            ...
            # # _logger.info(f"correos_validos {correos_validos}************")
    ...
    def descarga_vencimientos(self, month=None, year=None, hojas=None, control=None):
        ...
        status, month, year, hojas, mensajes_whatsapp, control_mails, id_mb, google_drive_config, creds, gc, servicio_drive, lista_archivos, pathgdrive, pathglocal, ejecucion = self.variables(month, year, hojas, control)
        duracion = 0
        _logger.info(f"status: {status} - month: {month} - year: {year} - hojas: {hojas} - mensajes_whatsapp: {mensajes_whatsapp} - control_mails: {control_mails} - id_mb: {id_mb} - google_drive_config: {google_drive_config} ************")
        ...
        if (status == 'idle' or status == 'inactivo') or status == '' or status == None:
            ...
            try:
                ...
                for hoja in hojas:
                    ...
                    df_sheets, sheet, sheet_df_dict, lista_archivos = google_drive_config.cargar_hoja(hoja, servicio_drive, gc, pathgdrive, mes_FILTRO)
                    result = self.actualizar_modelo_vencimientos(df_sheets, year, mes_FILTRO, hoja)
                    _logger.info(f"Actualizado modelo vencimientos {hoja}: {result} ************")
                ...
                for hoja in hojas:
                    ...
                    for record in self.search([('ramo', '=', hoja), ('mes', '=', mes_FILTRO), ('mail_status', '=', 'pendiente')]):
                        ...
                        if record.url and record.mail_status == 'pendiente' and record.ramo == hoja:
                            ...
                            if record.ramo in template_dic:
                                ...
                                record.enviar_correo(asunto, mensaje, template, record.ramo, control_mails)
                                if control_mails == 'marcar':
                                    record.write({'mail_status': 'enviado cliente'})
                                else:
                                    ...
                                    poliza_trm = str(record.id_poliza).lstrip('0')
                                    row = google_drive_config.buscar_fila(record.mes, poliza_trm, df_sheets)
                                    result = google_drive_config.update_cell(row, column_mail_status, 'enviado')
                                contador_enviados += 1
                            else:
                        ...
                        elif not record.url and record.mail_status == 'pendiente':
                ...
                google_drive_config.search([('cliente_id', '=', id_mb), ('clave', '=', 'status')]).write({'valor': 'enviando whatsapp'})  # Actualizar el estado a idle
    ...
    def enviar_correo(self, asunto, mensaje, template, ramo, control_mails='si'):
        self.ensure_one()
    ...
    def actualizar_sheet_vencimientos(self, google_drive_config, month=None, year=None, hojas=None):
