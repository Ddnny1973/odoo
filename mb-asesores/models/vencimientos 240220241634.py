...
class Vencimientos(models.Model):
    ...
    def send_mail_report(self, mes):
        ...
        selected_fields = ['ramo', 'id_poliza', 'dni_tomador', 'tomador', 'nombrecorto', 'formadepago',
                        'finvigencia', 'aviso', 'mes', 'year', 'movil', 'email_to', 'mensaje',
    ...
    def descarga_vencimientos(self, month=None, year=None, hojas=None):
        ...
        hojas = hojas.split(",")
        hojas = [a.strip().replace(" ", "_") for a in hojas]
        ...
        # Mostrar columnas del data frame lista_archivos
        # _logger.info(f"-----lista_archivos: {lista_archivos.columns}")
        # Listar archivos en la lista_archivos
        # for index, row in lista_archivos.iterrows():
        #     _logger.info(f"------nombrearchivo: {row['nombrearchivo']}")
        ...
        for hoja in hojas_corregido:
            ...
            if hoja in hojas:
                ...
                # # Listas las columnas del df
                # _logger.info(f"df_filtrado {hoja}: {df_filtrado.columns}")
                ...
                for index, row in df_filtrado.iterrows():
                    ...
                    if hoja == 'BOLIVAR':
                        ...
                        # if row['ESTADO'] == 'RENOVADA':
                        #     nombre_archivo = f"RENOVACION_{row['POLIZA']}.pdf"
                        # else:
                        #     nombre_archivo = f"{row['ESTADO']}_{row['POLIZA']}.pdf"
                        nombre_archivo = f"*_{row['POLIZA']}.pdf"
                    elif hoja == 'GENERALES':
                        nombre_archivo = f"RENOVACION_{row['POLIZA']}.pdf"
                    else:
                    ...
                    if 'TIPO DE PLAN' in df_filtrado.columns:
                        ...
                        tipo_plan = 'GENERAL'
                    mensaje = f"""{saludo} Sr(a). {row['NOMBRE CORTO']} \n
Para Mesa Barreneche usted es muy importante, por este medio y al correo {row['CORREO']},  {mensaje_especifico}, con inicio de vigencia {row['FIN DE VIGENCIA']}.\n
...
PSE- Tarjeta de Crédito o efectivo si lo prefiere.
                    ...
                    if hoja == 'PROVISION':
                        if row['ESTADO'].upper() == 'CONDICIONES':
                            ...
                            mensaje2 = f"""
                                        {patron} Sr(a). {row['TOMADOR']} \n
                                        Para Mesa Barreneche usted es muy importante, anexo enviamos las condiciones de renovación de la póliza que ampara su vehículo de placas {row['PLACA']}, con inicio de vigencia {row['FIN DE VIGENCIA']} con la compañía de seguros Mapfre. \n
                            ...
                            mensaje2 = f"""
                                        {patron} Sr(a). {row['TOMADOR']} \n
                                        Para Mesa Barreneche usted es muy importante, anexo enviamos la renovación de la póliza que ampara su vehículo de placas {row['PLACA']}, con inicio de vigencia {row['FIN DE VIGENCIA']} con la compañía de seguros Mapfre. \n
                                        Nuestra Asesora Comercial, Isabel Cristina Obando Cuartas, estará en contacto con toda la información que usted requiera. Celular 3206128289, correo electrónico comercial1@mbasesoresenseguros.com
                                        Gracias por contar con nosotros. 
                                        ¡¡Feliz Día!!"
                                        """
...
¡¡Feliz Día!!
        ...
        for hoja in hojas:
            for record in self.search([('aviso', '=', 'ok'), ('ramo', '=', hoja)]):
                ...
                if id_archivo_adjunto and record.mail_status == 'pendiente' and record.ramo == hoja:
                    _logger.info(f"Enviando correo electrónico: {record.id_poliza} - {record.tomador}")
                    ...
                    _logger.info(f"mail_enviado: {len(mail_enviado)} poliza {record.id_poliza} mes {record.mes}")
                    ...
                    if mail_enviado:
                        ...
                        template_autos = {
                                        ...
                                        'BASICO': 'mb-asesores.generico',
                                        'MOTOS': 'mb-asesores.generico',
                                        'CLASICOS': 'mb-asesores.generico',
                                        'CLASICO': 'mb-asesores.autosclasicos',
                                        'PESADOS': 'mb-asesores.generico'
                                    }
                        ...
                        if record.ramo in template_dic:
                            ...
                            record.enviar_correo(asunto, destinatario, mensaje, template, record.ramo)
                            record.write({'mail_status': 'enviado'})
                            contador_enviados += 1
                        else:
                            _logger.info(f"No se encontróel ramo: {record.ramo}")
                ...
                # Si no existe id_archivo_adjunto, actualizar el estado del correo a no disponible
                    ...
                    _logger.info(f"No se puedo procesar el correo")
                    url = ''
        ...
        json = {
            ...
            'registros_enviados': contador_enviados,
            'registros_no_enviados': contador_no_enviados,
        ...
        # ,
        #     'Tipo mes recibido': Tipo_mes_recibido,
        #     'Tipo year recibido': Tipo_year_recibido
    ...
    def enviar_correo(self, asunto, destinatario, mensaje, template, ramo):
        self.ensure_one()
