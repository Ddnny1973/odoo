    ...
    def action_cargar(self):
        ...
        for i, b64 in enumerate(archivos, 1):
            try:
                ...
                # Eliminar también pagos huérfanos que quedaron en los hitos
                # (pagos sin movimiento contable asociado de cargas anteriores)
                pagos_huerfanos = self.env['sicone.pago'].search([
                    ('hito_id.proyecto_id', '=', proyecto.id),
                ])
                pagos_huerfanos.unlink()
                ...
                for pago in pagos:
                    ...
                    # Prorrateo: el excedente fluye al siguiente hito.
                    # La creación del hito adicional se hace ANTES de crear pagos
                    # para evitar conflictos de recompute en Odoo.
                    primer_pago = True
                    while monto_restante > 0:
                        if indice_hito < len(hitos_list):
                            ...
                            # Sin hitos: buscar el adicional ya existente en hitos_list
                            hito_adicional = next(
                                (h for h in hitos_list
                                 if h.nombre == 'Pagos y Obras Adicionales'), None
                            )
                            if not hito_adicional:
                                # Buscar en BD por si ya existe de carga anterior
                                hito_adicional = self.env['sicone.hito'].search([
                                    ('proyecto_id', '=', proyecto.id),
                                    ('nombre', '=', 'Pagos y Obras Adicionales'),
                                ], limit=1)
                                if not hito_adicional:
                                    ultima_sec = max(
                                        (h.secuencia for h in hitos_list), default=0
                                    )
                                    # Crear con monto simbólico — se actualizará después
                                    hito_adicional = self.env['sicone.hito'].create({
                                        'proyecto_id': proyecto.id,
                                        'nombre': 'Pagos y Obras Adicionales',
                                        'secuencia': ultima_sec + 10,
                                        'monto': 0.01,
                                        'estado': 'pendiente',
                                        'contrato_ref': 'ambos',
                                    })
                                hitos_list.append(hito_adicional)
                                cobrado_hito[hito_adicional.id] = 0.0
                            # Actualizar monto del hito adicional para acomodar el excedente
                            hito_adicional.monto = cobrado_hito.get(
                                hito_adicional.id, 0.0) + monto_restante
                            hito = hito_adicional
                            aplicar = monto_restante
                            cobrado_hito[hito.id] = cobrado_hito.get(hito.id, 0.0) + aplicar
                            monto_restante = 0
